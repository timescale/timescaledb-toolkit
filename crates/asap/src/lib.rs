...
pub fn asap_smooth(data: &Vec<f64>, resolution: u32) -> Vec<f64> {
    ...
    for i in (0..peaks.len()).rev() {
        ...
        if w < lb || w == 1 {
            break;
        } else if (1.0 - acf.correlations[w as usize]).sqrt() * window_size as f64 > (1.0 - acf.correlations[window_size as usize]).sqrt() * w as f64 {
            continue;
        }
        ...
        if metrics.kurtosis() >= original_kurt {
            ...
            let test_lb = w as f64 * (acf.max_acf - 1.0).sqrt() / (acf.correlations[w as usize] - 1.0);
...
fn binary_search(head: u32, tail:u32, data: &Vec<f64>, min_obj: f64, original_kurt: f64, window_size: u32) -> u32 {
    ...
    while head <= tail {
        ...
        if metrics.kurtosis() >= original_kurt { /* Search second half if feasible */
            let roughness = metrics.roughness();
            if roughness < min_obj {
                window_size = w;
                min_obj = roughness;
