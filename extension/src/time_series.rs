...
#[pg_extern(schema = "toolkit_experimental", immutable, parallel_safe)]
pub fn normalize<'input> (
    mut series: toolkit_experimental::TimeSeries<'input>,
    interval: Interval,
    method: String,
    truncate: Option<bool>,
    ...
    unsafe {
        ...
        if !series.is_sorted() {
            ...
            match &mut series.series {
                ExplicitSeries { num_points: _, points } =>
                    points.as_owned().sort_by_key(|point| point.ts),
                NormalSeries { .. }
                | SortedSeries { .. }
                | GappyNormalSeries { .. }=> unreachable!(),
            }
        }
        ...
        let start = match range_start {
            Some(t) => t,
            None => series.iter().next().unwrap().ts,
        } / align * align;

        let end = match range_end {
            Some(t) => t,
            // TODO ensure this is constant time
            None => series.iter().last().unwrap().ts,
        } / align * align;
        ...
        let mut values = vec![starting_point.val];
        let mut next = start + interval;

        while next < first.ts {
            values.push(method.predict_left(next, first, Some(second)).val);
            next += interval;
        }
        ...
        while next <= end {
            ...
            while next < right.ts && next <= end {
                values.push(method.gapfill(next, left, right).val);
                next += interval;
            }
            if iter.peek().is_some() {
                ...
                while next <= end {
                    ...
                    values.push(method.predict_right(next, right, Some(left)).val);
                    next += interval;
                }
            }
        }
