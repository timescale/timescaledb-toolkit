...
impl<'a> Timevector<'a> {
    pub fn iter(&self) -> Iter<'_> {
        Iter::Slice{iter: self.points.iter()}
    }
...
pub fn timevector_trans_inner(
    ...
    unsafe {
        in_aggregate_context(fcinfo, || {
            ...
            state.points.as_owned().push(TSPoint{ts: time, val: value});
            state.num_points += 1;
            Some(state)
        })
    }
        ...
        in_aggregate_context(fcinfo, || {
            match (state, series) {
                ...
                (Some(mut state), Some(series)) => {
                    state.is_sorted = state.is_sorted && series.is_sorted &&
                        state.points.as_slice().last().unwrap().ts <= series.points.as_slice().first().unwrap().ts;
                    state.points.as_owned().extend_from_slice(series.points.slice());
                    Some(state)
                }
            }
        })
...
pub fn combine(first: Timevector<'_>, second: Timevector<'_>) -> Timevector<'static> {
    ...
    let is_sorted = first.is_sorted && second.is_sorted && 
        first.points.as_slice().last().unwrap().ts <= second.points.as_slice().first().unwrap().ts;
