    ...
    unsafe {
        in_memory_context(mctx, || {
            match state1 {
                None => match state2 {
                    None => None,
                    Some(state2) => Some(state2.clone().into()),
                },
                Some(mut state1) => match state2 {
                    None => Some(state1.clone().into()),
                    Some(mut state2) => {
                        state1.digest();
                        state2.digest();

                        // TODO This clone is stupid, implement a tdigest.merge
...
mod tests {
    ...
    fn apx_eql(value: f64, expected: f64) {
        assert!((value - expected).abs() / expected < 0.01, "Float value {} differs from expected {} by more than 1%", value, expected);
    }

    #[pg_test]
    fn test_hello_statscale() {
        assert_eq!("Hello, statscale", hello_statscale());
    }
    ...
    fn test_aggregate() {
        Spi::execute(|client| {
            ...
            apx_eql(min.unwrap(), 0.01);
            apx_eql(max.unwrap(), 100.0);
            apx_eql(count.unwrap(), 10000.0);
            ...
            apx_eql(mean.unwrap(), 50.005);
            apx_eql(sum.unwrap(), 500050.0);
            ...
            for i in 1..=100 {
                ...
                apx_eql(est_val.unwrap(), value);
                apx_eql(est_quant.unwrap(), quantile);
            }
        });
    }
}
